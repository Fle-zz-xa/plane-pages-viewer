...
export function WikiSidebar({
  pages, allPages, selectedPageId, loading, error,
  api, onSelectPage, onRefresh, onShowSettings,
}: Props) {
    ...
    <>
      <aside className="w-72 flex flex-col bg-white border-r border-gray-200 shrink-0">
        ...
        <div className="px-4 pt-5 pb-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onShowSettings} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors" title="Instellingen">
        ...
        <div className="px-3 pb-2 space-y-1">
          <Link href="/projects" className="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors border border-gray-100 group">
          ...
          <Link href="/my-tasks" className="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors border border-gray-100 group">
        ...
        <div className="px-3 pb-1"><div className="h-px bg-gray-100" /></div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-1">
            ...
            <div className="mx-3 my-4 bg-red-50 border border-red-200 rounded-xl p-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <button onClick={onShowSettings} className="mt-2 text-xs text-red-700 underline hover:text-red-900">
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="text-center py-12 text-gray-400">
              <p className="text-sm">{query ? 'Geen resultaten gevonden' : 'Nog geen paginas'}</p>
            </div>
          )}
      ...
      {showCreate && (
        <CreatePageModal
          ...
          onCreated={(id) => { setShowCreate(false); onRefresh(id); }}
        />
      )}
    </>
  );
